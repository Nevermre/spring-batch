...
public class FirstItemProcessor implements ItemProcessor<Student, com.infybuzz.mysql.entity.Student> {
	...
	/*
	 * @Override public StudentJson process(StudentJdbc item) throws Exception {
	 * System.out.println("Inside Item Processor"); StudentJson studentJson = new
	 * StudentJson(); studentJson.setID(item.getId());
	 * studentJson.setFirstName(item.getFirstName());
	 * studentJson.setLastName(item.getLastName());
	 * studentJson.setEmail(item.getEmail()); return studentJson; }
	 */

	@Override
	public com.infybuzz.mysql.entity.Student process(Student item) throws Exception {
		com.infybuzz.mysql.entity.Student student = 
				new com.infybuzz.mysql.entity.Student();
