 ...
 * list = new ArrayList<>();
 * 
 * for (StudentResponse sr : studentResponseArray) { list.add(sr); } return
 * list; }
 * 
 * public StudentResponse getStudent(long id, String name) {
 * System.out.println("id = " + id + " name = " + name); if (list == null) {
 * restCallToGetStudents(); } if (list != null && !list.isEmpty()) { return
 * list.remove(0); }
 * 
 * return null; } public StudentResponse restCallToCreateStudent( StudentCsv
 * studentCsv) { RestTemplate restTemplate = new RestTemplate(); return
